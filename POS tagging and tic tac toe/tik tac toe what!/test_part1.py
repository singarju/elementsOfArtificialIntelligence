...
def test_game_random_agent_start_second():
    board = "........."
    assert play_game(board, 3, 3, 3, player_first=False) == True
...
time_ = 300
@pytest.mark.timeout(time_)
def test_trivial_case0():
    simple_case = "xx.xx..x"
    correct_board = "xx.xx.xx"
    board = solver(simple_case, 4, 2, 3)
    board = parse_board_grid_to_string(board)
    assert board == correct_board

time_ = 300
@pytest.mark.timeout(time_)
def test_trivial_case1():
    board = "xx......................."
    assert play_game(board, 5, 5, 3, player_first=True) == True
    
    
time_ = 300
@pytest.mark.timeout(time_)
def test_trivial_case2():
    simple_case = "xx.x.xx....."
    correct_board = "xx.x.xx.x..."
    board = solver(simple_case, 3, 4, 3)
    board = parse_board_grid_to_string(board)
    assert board == correct_board
...
time_ = 300
@pytest.mark.timeout(time_)
def test_large_6x6_board():
    board = "." * 36
    assert play_game(board, 6, 6, 4, player_first=True) == True

time_ = 300
@pytest.mark.timeout(time_)
def test_blocking_almost_full_board():
    board = "x.xx.x..." 
    assert play_game(board, 3, 3, 3, player_first=True) == True
...
time_ = 300
@pytest.mark.timeout(time_)
def test_mostly_empty_11x11_board():
   board =  "xx......................."
   assert play_game(board, 5, 5, 3, player_first=True) == True
